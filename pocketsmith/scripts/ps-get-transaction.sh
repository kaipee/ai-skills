#!/bin/sh
# ps-get-transaction.sh — Get a single PocketSmith transaction by ID
# Usage: ps-get-transaction.sh --transaction-id ID [--api-key KEY] [--help]
...
usage() {
  cat <<EOF
Usage: ps-get-transaction.sh [OPTIONS]

Fetches full details for a single transaction via GET /transactions/{id}.
...
if [ -z "$TXN_ID" ]; then
  echo "ERROR: --transaction-id is required." >&2; usage >&2; exit 2
fi
