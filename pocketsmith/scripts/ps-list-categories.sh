#!/bin/sh
# ps-list-categories.sh — List all categories for a PocketSmith user
# Usage: ps-list-categories.sh [--user-id ID] [--api-key KEY] [--help]
...
usage() {
  cat <<EOF
Usage: ps-list-categories.sh [OPTIONS]

Lists the full category hierarchy via GET /users/{id}/categories.
Top-level categories include their children in the response.
...
HTTP_RESPONSE=$(curl -s -w "\n%{http_code}" \
  ...
  "$PS_BASE_URL/users/$PS_USER_ID/categories")
...
if command -v jq >/dev/null 2>&1; then
  echo "$HTTP_BODY" | jq '[.[] | {id, title, colour, parent_id, is_transfer, children: [.children[]? | {id, title, colour}]}]'
else
  echo "$HTTP_BODY"
fi
