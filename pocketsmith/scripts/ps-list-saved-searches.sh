#!/bin/sh
# ps-list-saved-searches.sh — List saved transaction searches for a PocketSmith user
# Usage: ps-list-saved-searches.sh [--user-id ID] [--api-key KEY] [--help]
...
usage() {
  cat <<EOF
Usage: ps-list-saved-searches.sh [OPTIONS]

Lists all saved transaction search filters via GET /users/{id}/saved-searches.
...
HTTP_RESPONSE=$(curl -s -w "\n%{http_code}" \
  ...
  "$PS_BASE_URL/users/$PS_USER_ID/saved-searches")
