#!/bin/sh
# ps-list-budget.sh — List budget analysis per category for a PocketSmith user
# Usage: ps-list-budget.sh [--user-id ID] [--api-key KEY] [--help]
...
usage() {
  cat <<EOF
Usage: ps-list-budget.sh [OPTIONS]

Returns budget analysis per category via GET /users/{id}/budget.
Shows budgeted amounts, actuals, and variance for the current forecast period.
...
HTTP_RESPONSE=$(curl -s -w "\n%{http_code}" \
  ...
  "$PS_BASE_URL/users/$PS_USER_ID/budget")
