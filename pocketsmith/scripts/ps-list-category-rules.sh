#!/bin/sh
# ps-list-category-rules.sh — List category rules for a PocketSmith user
# Usage: ps-list-category-rules.sh [--user-id ID] [--api-key KEY] [--help]
...
usage() {
  cat <<EOF
Usage: ps-list-category-rules.sh [OPTIONS]

Lists all auto-categorisation rules via GET /users/{id}/category-rules.
...
HTTP_RESPONSE=$(curl -s -w "\n%{http_code}" \
  ...
  "$PS_BASE_URL/users/$PS_USER_ID/category-rules")
...
if command -v jq >/dev/null 2>&1; then
  echo "$HTTP_BODY" | jq '[.[] | {id, payee_matches, category: .category.title, category_id: .category.id, apply_to_all, apply_to_uncategorised}]'
else
  echo "$HTTP_BODY"
fi
