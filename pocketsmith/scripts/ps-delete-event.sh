#!/bin/sh
# ps-delete-event.sh — Delete a PocketSmith budget forecast event
...
usage() {
  cat <<EOF
Usage: ps-delete-event.sh [OPTIONS]

Permanently deletes a budget forecast event via DELETE /events/{id}.

REQUIRES --confirm flag. This operation is IRREVERSIBLE.

Options:
  --event-id ID   Event ID to delete (required)
...
printf '%s\tDELETED\tdelete_event\t{"event_id":%s}\n' "$TIMESTAMP" "$EVENT_ID" >> "$AUDIT_LOG" 2>/dev/null || true
