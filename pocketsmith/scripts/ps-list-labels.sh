#!/bin/sh
# ps-list-labels.sh — List all transaction labels for a PocketSmith user
# Usage: ps-list-labels.sh [--user-id ID] [--api-key KEY] [--help]
...
usage() {
  cat <<EOF
Usage: ps-list-labels.sh [OPTIONS]

Lists all transaction labels via GET /users/{id}/labels.
...
HTTP_RESPONSE=$(curl -s -w "\n%{http_code}" \
  ...
  "$PS_BASE_URL/users/$PS_USER_ID/labels")
